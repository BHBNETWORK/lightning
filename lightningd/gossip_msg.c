...
void fromwire_gossip_getroute_entry(const u8 **pptr, size_t *max, struct route_hop *entry)
{
	fromwire_pubkey(pptr, max, &entry->nodeid);
	entry->amount = fromwire_u32(pptr, max);
	entry->delay = fromwire_u32(pptr, max);
}
void towire_gossip_getroute_entry(u8 **pptr, const struct route_hop *entry)
{
	towire_pubkey(pptr, &entry->nodeid);
